-- =============================================
-- Stored Procedures para Dashboard de NexusFinance - CORREGIDO
-- =============================================
...
BEGIN
    ...
    -- Obtener estadísticas de transacciones del mes actual
    SELECT 
...
BEGIN
    ...
    MesesBase AS (
        SELECT 
            ...
            CASE MONTH(DATEADD(MONTH, -MesOffset, GETDATE()))
                WHEN 1 THEN 'Ene ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 2 THEN 'Feb ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 3 THEN 'Mar ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 4 THEN 'Abr ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 5 THEN 'May ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 6 THEN 'Jun ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 7 THEN 'Jul ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 8 THEN 'Ago ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 9 THEN 'Sep ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 10 THEN 'Oct ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 11 THEN 'Nov ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
                WHEN 12 THEN 'Dic ' + CAST(YEAR(DATEADD(MONTH, -MesOffset, GETDATE())) AS VARCHAR(4))
            END AS MesNombre
        FROM Meses
    )
    SELECT 
...
IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'IX_Transacciones_Usuario_Fecha_Tipo' AND object_id = OBJECT_ID('Transacciones'))
BEGIN
...
IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'IX_MetasFinancieras_Usuario_Estado' AND object_id = OBJECT_ID('MetasFinancieras'))
BEGIN
...
IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'IX_Cuentas_Usuario_Activo' AND object_id = OBJECT_ID('Cuentas'))
BEGIN
...
PRINT 'Stored procedures del Dashboard creados exitosamente:';
PRINT '- sp_ObtenerEstadisticasGenerales';
PRINT '- sp_ObtenerTransaccionesPorCategoria';
PRINT '- sp_ObtenerTendenciasMensuales';
PRINT '- sp_ObtenerResumenMetas';
PRINT '- sp_ObtenerResumenCuentas';
PRINT '- sp_ObtenerTransaccionesRecientes';
PRINT '- sp_ObtenerDashboardCompleto';
PRINT '';
PRINT 'Índices de performance creados para optimizar las consultas.';
PRINT 'El Dashboard está listo para usar.';